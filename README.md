...
## DeFi Adapter for Alpha Homora

Alpha Homora defi adapter development compatible with Opty-Fi's earn-protocol. Utilises IAdapter.sol and IAdapterBorrow.sol interfaces to interact with Alpha Homora's Homora Bank (IBank.sol). HomoraBank then co-ordinates with the lending/LP pool via 'spells'. This defi adpater allows use of Alpha Homora 'spells' by Opty-Fi users. Published on Rinkeby Testnet. Alpha token address is supplied, other pools unavialable on Rinkeby and will be updated at the time of mainnet submission,


## Key changes

``` AlphaFinanceAdapter.sol ``` contains the main adapter code. It inherits from IAdapter.sol and IAdapterBorrow.sol interfaces, as well as all interfaces of Alpha Homomra v2 protocol (live on ethereum mainnet). The functionality of Opty-Fi adapters has been matched to the corresponding spells in Alpha v2's IBank contract.
